...
func buildProxy(v reflect.Value, buildFunc func(funcField reflect.StructField, field reflect.Value) func(arg ProxyArg) []reflect.Value) {
	...
	for i := 0; i < count; i++ {
		...
		if f.CanSet() {
			...
			case reflect.Func:
				if buildFunc != nil {
					buildMethod(v, f, ft, sf, buildFunc(sf, f))
				}
			}
		}
	}
...
func buildMethod(source reflect.Value, f reflect.Value, ft reflect.Type, sf reflect.StructField, proxyFunc func(arg ProxyArg) []reflect.Value) {
