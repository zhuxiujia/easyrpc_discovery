...
/*
	...
	"Arith.Divide".  To invoke one, a client first dials the server:
		...
		client, err := easyrpc.DialHTTP("tcp", serverAddress + ":1234")
		if err != nil {
...
func (server *Server) register(rcvr interface{}, name string, useName bool, deferFunc func(arg interface{}) string) error {
	...
	if sname == "" {
		s := "easyrpc.Register: no service name for type " + s.typ.String()
		log.Print(s)
		return errors.New(s)
	}
	if !isExported(sname) && !useName {
		s := "easyrpc.Register: type " + sname + " is not exported"
		log.Print(s)
		return errors.New(s)
	}
	s.name = sname
	...
	if len(s.method) == 0 {
		...
		if len(method) != 0 {
			...
			str = "easyrpc.Register: type " + sname + " has no exported methods of suitable type"
		}
		log.Print(str)
...
func (server *Server) Accept(lis net.Listener) {
	for {
		conn, err := lis.Accept()
		if err != nil {
			log.Print("easyrpc.Serve: accept:", err.Error())
			return
		}
